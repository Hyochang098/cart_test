package com.example.cart.display.service;
...
public class CartService {
    ...
    private final LoadingCacheStore<Long, SkuDisplayInfo> skuDisplayCache;
    ...
    public CartService(
        ...
        LoadingCacheStore<Long, SkuDisplayInfo> skuDisplayCache
    ) {
        ...
        this.skuDisplayCache = skuDisplayCache;
    }
    ...
    public CartResponse getCart(Long memberId) {
        ...
        for (CartStateItem stateItem : cartState.items()) {
            SkuDisplayInfo displayInfo = skuDisplayCache.get(stateItem.skuId(), this::loadSkuDisplayInfo);
            int finalPrice = displayInfo.price();
    ...
    private SkuDisplayInfo loadSkuDisplayInfo(Long skuId) {
        Sku sku = skuRepository.findById(skuId)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "SKU를 찾을 수 없습니다."));
        String imageUrl = skuImageRepository.findBySku_SkuIdIn(List.of(skuId)).stream()
            .findFirst()
            .map(SkuImage::getImage)
            .orElse("");
        return new SkuDisplayInfo(
            sku.getSkuId(),
            sku.getSkuName(),
            sku.getPrice(),
            imageUrl,
